...
function DonutTooltip({
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white/95 px-3 py-2 shadow-sm">
      <p className="text-[11px] font-medium text-black">{item.label}</p>
      <p className="mt-0.5 text-[11px] tabular-nums text-black">
        {value.toLocaleString("pt-BR")}{" "}
        <span className="ml-1 text-[11px] text-black font-medium">
          ({item.percentage.toFixed(0)}%)
        </span>
      </p>
      <p className="mt-1 text-[10px] text-black/70">Valor de vendas</p>
    </div>
  );
}
          ...
          {enriched.map((item, index) => {
            ...
            return (
              <Button
                ...
                className={`flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-left h-auto ${
                  isActive
